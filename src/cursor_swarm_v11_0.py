...
def ensure_readme_file():
    ...
    refresh_old = document_needs_version_refresh(
        README_FILE,
        "CursorSwarm v11.0",
        ["CursorSwarm v10.8", "CursorSwarm v10.9"],
    )
...
def ensure_release_documents():
    ...
    old_markers = ["CursorSwarm v10.8", "CursorSwarm v10.9", "Effective Version: CursorSwarm v10.9"]
    ...
    generate_release_notes_file(
        show_message=False,
        overwrite=document_needs_version_refresh(RELEASE_NOTES_FILE, "CursorSwarm v11.0 Release Notes", old_markers),
    )
...
def make_section(parent, title):
    """Create a modern CustomTkinter card section for v11.0."""
    outer = ctk.CTkFrame(
        parent,
        corner_radius=14,
        border_width=1,
...
def ui_spinbox(parent, variable, from_, to, *, increment=1, width=8):
    ...
    # Spinbox is dark-styled and embedded inside the v11.0 CTk cards.
    return tk.Spinbox(
        parent,
        from_=from_,
        to=to,
...
control_panel.title("Cursor Swarm v11.0 Control Panel")
...
ctk.CTkLabel(
    brand_box,
    text="CustomTkinter control panel • same Store-approved cursor engine",
    font=("Segoe UI", 12),
    text_color="#94a3b8",
    anchor="w",
...
ui_slider(drawn_box, real_cursor_scale_var, 0.50, 1.50, lambda value: real_cursor_scale_label_var.set(f"{float(value):.2f}x"), width=250).grid(row=3, column=1, sticky="w", padx=8, pady=5)
...
ui_label(preset_io_box, "Full backup includes custom presets and app settings. Single preset import overwrites only the selected slot.", wraplength=700).grid(row=3, column=0, columnspan=3, sticky="w", padx=8, pady=5)
...
ui_button(startup_box, "Save Startup Settings", ui_save_startup_settings, width=190).grid(row=3, column=0, padx=8, pady=7)
ui_button(startup_box, "Apply Startup Mode Now", ui_apply_startup_now, secondary=True, width=205).grid(row=3, column=1, padx=8, pady=7)
ui_button(startup_box, "Generate Fallback Restore Script", generate_emergency_restore_script, secondary=True, width=260).grid(row=4, column=0, columnspan=2, sticky="w", padx=8, pady=7)
ui_label(startup_box, "Safe Mode starts with the Windows cursor visible, mirror disabled, and swarm paused.\nLast Used State is saved when you quit safely.", wraplength=700).grid(row=5, column=0, columnspan=3, sticky="w", padx=8, pady=5)
...
about_text = (
    ...
    "v11.0 updates the Control Panel to a modern CustomTkinter interface while keeping the Store-approved cursor engine, floating CS button, shortcut hints, and recovery system intact.\n\n"
    "Main hotkeys:\n"
...
ui_button(config_box, "Generate Fallback Restore Script", generate_emergency_restore_script, secondary=True, width=260).grid(row=3, column=0, sticky="w", padx=8, pady=7)
ui_button(config_box, "Create README", generate_readme_file, secondary=True).grid(row=3, column=1, sticky="w", padx=8, pady=7)
...
def update():
    ...
    if show_startup_help and now - start_time < 10:
        canvas.create_text(
            SCREEN_W // 2,
            35,
            text="CursorSwarm v11.0 — Ctrl+Alt+C panel | Ctrl+Alt+Space Safe Mode | Ctrl+Alt+Q Quit Safely",
            fill="white",
            font=("Arial", 16, "bold")
        )
